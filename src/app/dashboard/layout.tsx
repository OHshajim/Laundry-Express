...
export default function CustomerDashboardLayout({ children }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-slate-50/60 w-full overflow-x-hidden">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 bg-white border-r border-slate-200/80 flex flex-col justify-between transition-transform duration-200 shrink-0 shadow-2xs lg:static lg:translate-x-0 ${
      ...
      >
            ...
            {DASHBOARD_NAV.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setMobileMenuOpen(false)}
                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 ${
                    active
        ...
        <div className="p-3.5 border-t border-slate-100 space-y-2">
          <Link href="/order" className="block">
            <Button size="sm" className="w-full bg-primary hover:bg-primary-dark text-white text-xs h-9 shadow-xs">
          ...
          <a
            ...
            className="flex items-center gap-1.5 p-2 rounded-lg bg-pink-50/50 text-[11px] text-primary font-bold hover:bg-pink-50 transition-colors"
          >
            <Phone className="h-3 w-3 shrink-0" />
            <span className="truncate">Support: {APP_CONFIG.supportPhone}</span>
          </a>
          ...
          actions={
            <div className="flex items-center gap-2">
              <a href={`tel:${APP_CONFIG.supportPhone}`}>
                <Button variant="outline" size="sm" className="text-xs border-slate-200 text-slate-700 h-8">
